<header class="public">
    ...
    <div id="category_menu" class="bg-gray-300 py-2 w-full hidden">
      <div class="container mx-auto text-gray-700 flex flex-wrap justify-center">
        @foreach ($categories as $category)
          <a href="/category/{{ $category->id }}" class="block hover:text-gray-600 px-4 py-4 w-32">{{ $category->category_title}}({{ $category->posts_count }})</a>
        @endforeach
      </div>
    </div>
  @endisset
  ...
  @isset ($tags)
    <div id="tag_menu" class="bg-gray-300 py-2 w-full hidden">
      <div class="container mx-auto text-gray-700 flex flex-wrap justify-center">
        @foreach ($tags as $tag)
          <a href="/tag/{{ $tag->id }}" class="block hover:text-gray-600 px-4 py-4 w-32">{{ $tag->tag_title}}({{ $tag->posts_count }})</a>
        @endforeach
      </div>
    </div>
  @endisset
      ...
      <div class="container mx-auto">
        <nav class="text-gray-700">
          ...
          <div class="flex flex-wrap justify-center">
            @foreach ($categories as $category)
              <a href="/category/{{ $category->id }}" class="block hover:text-gray-600 sm:border-0 border border-gray-500 rounded-full px-4 py-4 sm:w-32 w-auto sm:mb-0 mb-2 sm:mr-0 mr-2">{{ $category->category_title}}({{ $category->posts_count }})</a>
            @endforeach
          </div>
          ...
          <div class="flex flex-wrap justify-center">
            @foreach ($tags as $tag)
                  <a href="/tag/{{ $tag->id }}" class="block hover:text-gray-600 sm:border-0 border border-gray-500 rounded-full px-4 py-4 sm:w-32 w-auto sm:mb-0 mb-2 sm:mr-0 mr-2">{{ $tag->tag_title}}({{ $tag->posts_count }})</a>
            @endforeach
