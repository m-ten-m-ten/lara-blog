...
<div class="container">
  <div class="flex items-center">
    <h1>カテゴリー一覧</h1>
    <a class="btn-white" href="/dashboard/category/create">カテゴリー追加</a>
  </div>
    ...
    <table class="table-auto w-full border mb-4">
      ...
      <tbody id="boxes">
        ...
        <tr class="{{ $loop->odd ? 'bg-gray-100' : ''}}">
          ...
          <td><a class="text-blue-700 font-bold" href="/dashboard/category/edit/{{ $category->id }}">{{ $category->category_title }}</a></td>
          <td>{{ $category->category_name }}</td>
          <td>
            <a class="text-blue-700 underline" href="/dashboard/category/edit/{{ $category->id }}">編集</a>
            <span class="md:inline hidden">|</span>
            <button class="text-link md:inline block md:mt-0 mt-1" type="submit" name="deleteId" value="{{$category->id}}" onClick="delete_alert(event);return false;">削除</button>
          </td>
        </tr>
        @endforeach
      </tbody>
